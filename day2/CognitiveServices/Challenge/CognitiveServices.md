...
The Custom Vision Service has 2 types of endpoints. One for training the model and one for running predictions against the model. For this example, we will use both. Therfore, select *both*. Fill in the *name* and the *region* as well as the *pricing tier* for the training and the prediction resource:
